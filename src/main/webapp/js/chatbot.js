...
let adminOnline = true; // Cambia a false si no hay admin disponible
let liveChatHistory = [];
...
function openLiveChat() {
    ...
    // Mensaje inicial del admin (simulado)
    if (liveChatHistory.length === 0) {
        setTimeout(() => {
            addLiveAdminMessage('¡Hola! Soy un administrativo del TecNM Los Mochis. ¿En qué puedo ayudarte hoy?');
        }, 1000);
    }
...
// ============================================
// ENVIAR MENSAJE EN CHAT EN VIVO
// ============================================
...
function sendLiveChatQuick(message) {
    document.getElementById('liveChatInput').value = message;
    sendLiveChatMessage();
}

// ============================================
// AGREGAR MENSAJE DEL USUARIO (CHAT EN VIVO)
// ============================================
...
function addLiveUserMessage(text) {
    ...
    const welcome = messagesDiv.querySelector('.live-chat-welcome');
    if (welcome) {
        welcome.remove();
    }
    ...
    messageDiv.innerHTML = `
        ...
        <div class="live-message-content">
            ${escapeHtml(text)}
            <div class="live-message-time">${getCurrentTime()}</div>
        </div>
    `;
    messagesDiv.appendChild(messageDiv);
...
function addLiveAdminMessage(text) {
    const messagesDiv = document.getElementById('liveChatMessages');
...
function simulateAdminResponse(userMessage) {
    ...
    if (normalized.includes('inscri') || normalized.includes('registro')) {
        response = 'Con gusto te ayudo con información sobre inscripciones. El proceso inicia el 3 de febrero. ¿Necesitas información sobre requisitos o fechas específicas?';
    } else if (normalized.includes('beca')) {
        response = 'Tenemos múltiples opciones de becas disponibles. Te puedo enviar información detallada por correo. ¿Cuál es tu email?';
    } else if (normalized.includes('costo') || normalized.includes('precio')) {
        response = 'La inscripción inicial tiene un costo de $5,090 MXN. Las reinscripciones semestrales son aproximadamente $2,000-$3,500 MXN. ¿Necesitas información sobre formas de pago?';
    } else if (normalized.includes('carrera') || normalized.includes('licenciatura')) {
        response = 'Ofrecemos 11 licenciaturas en la unidad Los Mochis. ¿Te interesa alguna ingeniería en particular?';
    } else if (normalized.includes('calendario') || normalized.includes('fecha')) {
        response = 'El calendario escolar 2025 ya está disponible. Las clases inician el 27 de enero. ¿Necesitas el PDF completo?';
    } else if (normalized.includes('contacto') || normalized.includes('teléfono') || normalized.includes('telefono')) {
        response = 'Puedes contactarnos al (668) 812-4939 o (668) 815-0650. Nuestro horario es de lunes a viernes de 8:00 a 15:00 hrs. ¿Necesitas algo más?';
    } else if (normalized.includes('hola') || normalized.includes('buenos')) {
        response = '¡Hola! ¿En qué más puedo ayudarte el día de hoy? 😊';
    } else if (normalized.includes('gracias')) {
        response = '¡Con mucho gusto! Si tienes más preguntas, estoy aquí para ayudarte. 🐉 #SomosDragonesITLM';
    } else {
        response = 'Entiendo tu pregunta. Déjame verificar esa información. ¿Podrías darme más detalles o especificar tu duda?';
    }
...
function showAdminTyping() {
    ...
    if (messagesDiv.querySelector('.admin-typing')) return;
    
    const typingDiv = document.createElement('div');
    typingDiv.className = 'admin-typing active';
    typingDiv.innerHTML = `
        <div class="live-message-avatar">👨‍💼</div>
...
function hideAdminTyping() {
    const typingIndicator = document.querySelector('.admin-typing');
    if (typingIndicator) {
...
// ============================================
// NOTIFICACIÓN NUEVA (SIMULAR)
// ============================================

// Simular que llega un mensaje del admin después de 30 segundos
setTimeout(() => {
    if (!liveChatActive) {
        const badge = document.querySelector('.notification-badge');
        if (badge) {
...
console.log('💬 Sistema de chat en vivo activado');